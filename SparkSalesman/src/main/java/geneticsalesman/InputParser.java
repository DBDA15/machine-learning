...
public class InputParser implements Closeable {
	    ...
	    try(BufferedReader in = new BufferedReader(new InputStreamReader(createInputStream(citiesFile),StandardCharsets.UTF_8))) {
	    	...
	    	while((l=in.readLine())!=null) {
	    		...
	    		if(parts.length == 5 && Integer.parseInt(parts[2])>50000)
					citiesList.add(new City(id++, parts[1], Double.parseDouble(parts[4]), Double.parseDouble(parts[3])));
	    	}
	    }
	...
	private InputStream createInputStream(String fileName) throws IOException, URISyntaxException {
		if(fileName.startsWith("hdfs://")) {
			...
			FileSystem hdfs = FileSystem.get( getHost(fileName), configuration );
	...
	public static URI getHost(String path) throws URISyntaxException {
		return new URI(path.replace("hdfs://", "").split("/")[0]);
	}
